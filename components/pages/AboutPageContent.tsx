...
interface Props {
  lang: Lang;
  originLineMap: Record<string, string>;
  content: Record<string, Record<string, Record<string, string>>>;
}

export default function AboutPageContent({ lang, originLineMap, content }: Props) {
    ...
    <div style={{ background: "#0A0A0A", color: "#EDEDED" }}>
          ...
          <ScrollReveal direction="up" delay={300}>
            <p className="max-w-xl leading-relaxed text-lg mb-4 font-body" style={{ color: "#8A8A8A" }}>
              {content?.about?.hero?.subtitle ||
                (lang === 'ja'
                  ? "ネクストダイニングは数十年、数百年の歴史と哲学を持つ職人のブランドを発掘し、韓国と海外で直営運営するプレミアム外食グループです。"
                  : "넥스트다이닝은 수십 년, 수백 년 역사와 철학을 가진 장인의 브랜드를 발굴하고 한국과 해외에서 직영 운영하는 프리미엄 외식 그룹입니다.")}
            </p>
            ...
            >
              <p>
                {content?.about?.what_we_do?.paragraph_1 ||
                  (lang === 'ja'
                    ? "ネクストダイニングは外食ブランドを自ら立ち上げるのではなく、すでに数十年以上の歴史と哲学を持つ職人のブランドを発掘し、韓国で運営することを核心としています。"
                    : "넥스트다이닝은 외식 브랜드를 직접 만들기보다, 이미 수십 년 이상의 역사와 철학을 가진 장인의 브랜드를 발굴하고 한국에서 운영하는 것을 핵심으로 합니다.")}
              </p>
              <p>
                {content?.about?.what_we_do?.paragraph_2 ||
                  (lang === 'ja'
                    ? "国家公認キムチ名人が創立した봉우리 한정식、370年の伝統を持つ長崎の手延べ麺 진가와、長崎の職人 타카다 유지のとんかつ 분지로、九州1位の寿司屋 와카타케마루の韓国法人 다이센스시、ニューヨーク・マンハッタンのメディアアートダイニング NOFLEX NYCまで — ネクストダイニングが運営する10ブランドの背後には必ず検証されたストーリーがあります。"
              ...
              <p>
                {content?.about?.what_we_do?.paragraph_3 ||
                  (lang === 'ja'
                    ? "私たちの役割は、その本質を損なうことなく、韓国のお客様が最高の体験ができる空間とサービスを作ることです。"
            ...
            <div className="flex items-center justify-between mb-12">
              <div>
                <p
                  ...
                  style={{ color: "#C8A96E" }}
                >
                  {t(lang, 'about.portfolio_label')}
                </p>
                <h2 className="font-display text-3xl" style={{ color: "#EDEDED" }}>
                  {t(lang, 'about.portfolio_heading')}
                </h2>
              </div>
              <Link
                href={`${prefix}/brands`}
            ...
            return (
              <ScrollReveal key={cat} direction="up" delay={80}>
                <div className="mb-12 last:mb-0">
                  ...
                  >
                    {t(lang, `about.category.${cat}` as Parameters<typeof t>[1])}
                    <span className="text-sm font-normal" style={{ color: "#8A8A8A" }}>
                      {catBrands.length}{t(lang, 'about.brands_count')}
                    </span>
                  </h3>
                  <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                    {catBrands.map((brand) => {
                      ...
                      return (
                        ...
                        >
                          <div className="w-14 h-14 shrink-0 rounded-xl flex items-center justify-center p-2">
                            <Image
                              src={brand.logo}
                              alt={`${brand.name} 로고`}
                              width={48}
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="flex items-center gap-2 mb-1.5">
                              ...
                              >
                                {t(lang, `about.format.${brand.format}` as Parameters<typeof t>[1])}
                              </span>
                            </div>
                            ...
                            <div className="flex items-center gap-3 text-xs" style={{ color: "#8A8A8A" }}>
                              {activeCount > 0 && (
                                <span>{activeCount}{t(lang, 'about.stores_count')}</span>
                              )}
                            </div>
                          </div>
                        </Link>
      ...
      <section className="py-20" style={{ background: "#0F0F0F" }}>
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <ScrollReveal direction="up">
            <div className="max-w-3xl mb-12">
              ...
              <p className="leading-relaxed font-body" style={{ color: "#8A8A8A" }}>
                {lang === 'ja'
                  ? "ネクストダイニングはロッテワールドモール、新世界アウトレット、アイパークモール、サウンズ漢南など国内主要商業施設に出店運営しており、ニューヨーク・マンハッタンに海外直営店を運営しています。和食・韓食・洋食・カフェなど多業態ポートフォリオから、貴社の空間に最適なブランドをご提案いたします。"
                  : "넥스트다이닝은 롯데월드몰, 신세계아울렛, 아이파크몰, 사운즈한남 등 국내 주요 상업시설에 입점 운영 중이며, 뉴욕 맨해튼에 해외 직영점을 운영하고 있습니다. 일식·한식·양식·카페 등 다업태 포트폴리오에서 귀사 공간에 최적화된 브랜드를 제안드립니다."}
              </p>
            </div>
          </ScrollReveal>
          ...
          {(() => {
            const RAW_STATS = [
              content?.about?.partner_stats?.stat_1 ||
                (lang === 'ja' ? "10 | プレミアムブランド | 和食・韓食・洋食・カフェ" : "10개 | 프리미엄 브랜드 | 일식·한식·양식·카페"),
              content?.about?.partner_stats?.stat_2 ||
                (lang === 'ja' ? "21 | 全国直営店舗 | ソウル・水原・釜山・驪州・ニューヨーク" : "21 | 전국 직영 매장 | 서울·수원·부산·여주·뉴욕"),
              content?.about?.partner_stats?.stat_3 ||
                (lang === 'ja' ? "350億+ | 年間売上規模 | 2025年基準、全店舗直営" : "350억+ | 연 매출 규모 | 2025년 기준, 전 매장 직영"),
            ];
            const STAT_FALLBACKS = [
              { value: 10, suffix: lang === 'ja' ? "" : "개", label: lang === 'ja' ? "プレミアムブランド" : "프리미엄 브랜드", sub: lang === 'ja' ? "和食・韓食・洋食・カフェ" : "일식·한식·양식·카페" },
              { value: 21, suffix: "", label: lang === 'ja' ? "全国直営店舗" : "전국 직영 매장", sub: lang === 'ja' ? "ソウル・水原・釜山・驪州・ニューヨーク" : "서울·수원·부산·여주·뉴욕" },
              { value: 350, suffix: lang === 'ja' ? "億+" : "억+", label: lang === 'ja' ? "年間売上規模" : "연 매출 규모", sub: lang === 'ja' ? "2025年基準、全店舗直営" : "2025년 기준, 전 매장 직영" },
            ];
              ...
              const VALUE_FALLBACKS = [
                {
                  title: "Originality",
                  desc: lang === 'ja'
                    ? "検証された職人の伝統とオリジナリティを発掘し、保存します"
                    : "검증된 장인의 전통과 오리지널리티를 발굴하고 보존합니다",
                  icon: "M12 2L2 7l10 5 10-5-10-5zM2 17l10 5 10-5M2 12l10 5 10-5",
                },
                {
                  title: "Premium Quality",
                  desc: lang === 'ja'
                    ? "最高の食材と調理法で、妥協のない品質を追求します"
                    : "최상의 식재료와 조리법으로 타협 없는 품질을 추구합니다",
                  icon: "M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z",
                },
                {
                  title: lang === 'ja' ? "顧客中心のイノベーション" : "고객 중심 혁신",
                  desc: lang === 'ja'
                    ? "お客様の体験を最優先に置き、サービスと空間を設計します"
                    : "고객 경험을 최우선에 두고 서비스와 공간을 설계합니다",
                  icon: "M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2M9 11a4 4 0 1 0 0-8 4 4 0 0 0 0 8zM23 21v-2a4 4 0 0 0-3-3.87M16 3.13a4 4 0 0 1 0 7.75",
                },
                {
                  title: lang === 'ja' ? "運営の専門性" : "운영 전문성",
                  desc: lang === 'ja'
                    ? "データ駆動のシステムで多業態直営店舗を安定的に運営します"
                    : "데이터 기반 시스템으로 다업태 직영 매장을 안정적으로 운영합니다",
