...
const VALUES = [
  { title: "장인정신", desc: "타카다 유지 장인의 50년 노하우, 매일 새로 뽑는 소바. 정성은 기술보다 강합니다." },
  { title: "브랜드 경험", desc: "맛·공간·서비스를 하나의 이야기로 설계해 고객이 오래 기억하는 경험을 만듭니다." },
  { title: "사람 중심", desc: "직원이 행복해야 고객의 경험도 따뜻해집니다. 함께 성장하는 팀을 만들어갑니다." },
  { title: "본질의 가치", desc: "보이는 화려함보다 음식 본연의 깊이를 먼저 생각합니다." },
];
      ...
      <section className="relative bg-stone-950 text-white overflow-hidden">
        ...
        <div className="relative mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-24 lg:py-36">
          ...
          <h1 className="text-4xl sm:text-6xl font-bold leading-tight tracking-tight max-w-3xl">
            음식이 만드는<br />
            <span className="text-stone-300">새로운 기준</span>
          </h1>
          <p className="mt-6 text-lg text-stone-400 max-w-xl leading-relaxed">
            봉우리 한정식부터 나가사키 장인 돈카츠까지. 넥스트다이닝의 9개 브랜드는
            각자의 철학과 이야기로 한국 외식의 수준을 높입니다.
          </p>
          <div className="mt-10 flex flex-wrap gap-3">
            <Link href="/brands" className="bg-white text-stone-900 px-6 py-3 rounded-lg text-sm font-semibold hover:bg-stone-100 transition-colors">
              레스토랑 보기
            </Link>
            <Link href="/about" className="border border-stone-600 text-stone-300 px-6 py-3 rounded-lg text-sm font-semibold hover:border-stone-400 hover:text-white transition-colors">
              브랜드 스토리
            </Link>
          </div>
        </div>
      ...
      <section className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-20">
        <div className="flex items-end justify-between mb-10">
          <div>
            ...
            <h2 className="text-3xl font-bold">9개의 이야기</h2>
          </div>
          ...
          {activeBrands.map((brand) => (
            <Link key={brand.id} href={`/brands/${brand.id}`} className="group block rounded-2xl overflow-hidden border border-stone-100 hover:border-stone-300 hover:shadow-lg transition-all">
              ...
              <div className="p-5">
                ...
                <p className="text-sm text-stone-500 line-clamp-2">{brand.tagline}</p>
      ...
      <section className="bg-stone-50 py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <div className="text-center mb-12">
            ...
            <h2 className="text-3xl font-bold">넥스트다이닝이 지키는 것</h2>
          </div>
