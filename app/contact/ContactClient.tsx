...
function buildContactTypes(emails: CmsEmails) {
  return [
    {
      title: "입점·제휴 제안",
      icon: (
...
function ContactContent({ hero, emails }: ContactContentProps) {
  const CONTACT_TYPES = buildContactTypes(emails);
          ...
          <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
            ...
            <ScrollReveal stagger className="grid sm:grid-cols-3 gap-4 mb-16">
              {[
                ...
                { value: "21", label: "전국·해외 직영 매장", sub: "서울·수원·부산·여주·뉴욕" },
...
export default function ContactClient({ hero, emails }: ContactContentProps) {
  return (
    <Suspense>
      <ContactContent hero={hero} emails={emails} />
    </Suspense>
  );
}
